...
func traversalFieldType(fields []*protogen.Field, prefix string, outputMap map[string]interface{}, depth int) {
	if depth > 10 {
		// 添加深度限制，以避免无限递归
		return
	}
	...
	for _, field := range fields {
		...
		if field.Desc.Kind() == protoreflect.MessageKind {
			traversalFieldType(field.Message.Fields, filedKey, outputMap, depth+1)
		} else {
			// 做一下约束，否则太多字段生成。
...
func GetFiledTypeConfig(fields []*protogen.Field, prefix string) map[string]interface{} {
	...
	traversalFieldType(fields, prefix, outputMap, 0)
